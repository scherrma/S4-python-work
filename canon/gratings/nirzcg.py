...
class NIRZCG(Grating):
    si_n = interp.interp1d(*zip(*[[nu2lambda(float(f))*10**9, n] for (f, n) in h.opencsv('../matdat/silicon_n.csv',1)]))
    si_k = interp.interp1d(*zip(*[[nu2lambda(float(f))*10**9,n] for (f, n) in h.opencsv('../matdat/silicon_k.csv',1)]))
    ...
    def evaluate(self):
        if self.fom is None:
            ...
            for wl in np.linspace(*self.wls):
                ...
                S.SetMaterial('Oxide',complex(self.__class__.sio2_n(wl), self.__class__.sio2_k(wl))**2)
